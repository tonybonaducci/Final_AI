...
def torneo_binario(cr_1,cr_2,train_data,labels,atributes):
        ...
        """
        f_f_1, c_r_1, r_r_1 = utilities.weighted_fitness_function(train_data,labels,cr_1)
        f_f_2, c_r_2, r_r_2 = utilities.weighted_fitness_function(train_data,labels,cr_1)
...
def seleccion_AGE(poblacion,train_data,labels,atributes):

    orden_1 = rd.sample(range(tam_poblacion),2)
    orden_2 = rd.sample(range(tam_poblacion),2)
    orden = [orden_1,orden_2]

    padre_1 = torneo_binario(poblacion[orden_1[0]],poblacion[orden_1[1]],train_data,labels,atributes)
    padre_2 = torneo_binario(poblacion[orden_2[0]],poblacion[orden_2[1]],train_data,labels,atributes)

    nueva_poblacion = [torneo_binario(poblacion[orden[i][0]],poblacion[orden[i][1]],train_data,labels,atributes) for i in range(2)]
    
    return padre_1, padre_2


def seleccion(poblacion,train_data,labels,atributes):
    n_atr = len(atributes)
    ...
    for i in range(tam_poblacion-1):
        nueva_poblacion[i] = torneo_binario(poblacion[orden[i]],poblacion[orden[i+1]],train_data,labels,atributes)
        #print("Nueva poblacion ",i," = ",nueva_poblacion[i])
    nueva_poblacion[tam_poblacion - 1] = torneo_binario(poblacion[orden[0]],poblacion[orden[tam_poblacion - 1]],train_data,labels,atributes)
...
def valor_cromosoma(cromosoma,train_data,labels,atributes):

    f_f, c_r, r_r = utilities.weighted_fitness_function(train_data,labels,w)

    return f_f, c_r, r_r

def valor_poblacion(seeds,train_data,labels,atributes): 
    ...
    for i in range(tam_poblacion):
        #print(i)
        f_f, c_r, r_r = utilities.weighted_fitness_function(train_data,labels,seeds[i])
    ...
    return mejor, c_mejor, r_mejor, indice_mejor, indice_peor

def valor_poblacion_age(seeds,train_data,labels,atributes):
    #print("Valoramos la población: ",seeds)
    mejor = cota_inferior
    peor = cota_superior
    c_mejor = 0.0 
    ...
    return mejor, c_mejor, r_mejor, peor, segundo_peor, indice_mejor, indice_peor, indice_segundo_peor

def valor_cromosoma(seed,train_data,labels,atributes):
    
    f_f, c_r, r_r = utilities.weighted_fitness_function(train_data,labels,seed)

    return f_f  

def AGG_BLX(train_data,labels,weights,atributes):
    ...
    n_esperado_mutaciones = int(p_mutacion*tam_poblacion*n_atr)
    p_intermedia_cruce = seleccion(poblacion,train_data,labels,atributes) 

    mejor_fit, c_mejor, r_mejor, indice_mejor, indice_peor = valor_poblacion(p_intermedia_cruce,train_data,labels,atributes)

    n_evaluaciones = 1
    while(n_evaluaciones<15000):
        p_intermedia_cruce = seleccion(poblacion,train_data,labels,atributes) 
        ...
        nuevo_fit, nuevo_c_mejor, nuevo_r_mejor, nuevo_i_mejor, nuevo_i_peor = valor_poblacion(p_intermedia_mutacion,train_data,labels,atributes)
        ...
        if(nuevo_fit < mejor_fit):
            ...
            mejor_fit = nuevo_fit
            c_mejor = nuevo_c_mejor
            r_mejor = nuevo_r_mejor
        ...
        #print("Pasamos a la siguiente generación :",mejor_fit)
    return poblacion, mejor_fit, c_mejor, r_mejor, indice_mejor,poblacion[indice_mejor]

def AGG_aritmetico(train_data,labels,weights,atributes):
...
def AGE_BLX(train_data,labels,weights,atributes):
    ...
    #mejor_fit, peor, segundo_peor, indice_mejor, indice_peor, indice_segundo_peor = valor_poblacion_age(poblacion,train_data,labels,atributes)

    n_evaluaciones = 0
    while(n_evaluaciones<15000):
        #print("Partimos de la población:",poblacion)
        #--------seleccion--------
        padre_1, padre_2 = seleccion_AGE(poblacion,train_data,labels,atributes)
        ...
        mejor_fit,c_mejor, r_mejor, peor, segundo_peor, i_mejor, i_peor, i_segundo_peor = valor_poblacion_age(poblacion,train_data,labels,atributes)
        n_evaluaciones = n_evaluaciones + tam_poblacion

        v_hijo_1 = valor_cromosoma(p_intermedia[0],train_data,labels,atributes)
        v_hijo_2 = valor_cromosoma(p_intermedia[1],train_data,labels,atributes)
    ...
    return poblacion, mejor_fit, c_mejor, r_mejor, i_mejor,poblacion[i_mejor]


def AGE_aritmetico(train_data,labels,weights,atributes):
